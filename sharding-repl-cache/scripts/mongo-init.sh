...
# 8. Создание тестовых данных для кеширования
echo "Создание тестовых данных для кеширования..."
docker compose exec -T mongos_router mongosh --quiet <<EOF
use somedb
for (let i = 0; i < 100; i++) {
  db.users.insertOne({
...
# 9. Проверка скорости запросов
echo -e "\nТестирование скорости запросов:"
echo "Первый запрос (должен быть медленным):"
time curl -s "http://localhost:8080/users/users" > /dev/null

echo -e "\nВторой запрос (должен быть быстрым, закешированным):"
time curl -s "http://localhost:8080/users/users" > /dev/null
